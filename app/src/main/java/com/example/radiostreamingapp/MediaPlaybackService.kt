...
// Imports para manejo de hilos y reintentos
...
class MediaPlaybackService : Service() {
    ...
    override fun onCreate() {
        ...
        // Initialize ExoPlayer - CONFIGURACIÓN ORIGINAL SIMPLE
    ...
    fun playStation(station: RadioStation) {
        Logger.d(TAG, "Iniciando reproducción: ${station.name} - ${station.streamUrl}")

        // Cancelar reintentos anteriores
        reconnectJob?.cancel()
        retryCount = 0
        ...
        try {
            // Prepare MediaItem from URL - VERSIÓN ORIGINAL
            ...
            Logger.d(TAG, "Reproducción iniciada exitosamente para ${station.name}")

        } catch (e: Exception) {
            Logger.e(TAG, "Error al iniciar reproducción", e)
    ...
    private fun handlePlaybackError(errorMessage: String, errorType: ErrorType) {
        Logger.e(TAG, "Error de reproducción: $errorMessage")
        ...
        if (currentStation != null && retryCount < maxRetries) {
            ...
            Logger.d(TAG, "Reintentando en ${delayMs}ms (intento $retryCount/$maxRetries)")

            // USAR DISPATCHERS.MAIN PARA EVITAR PROBLEMAS DE HILO
            reconnectJob = serviceScope.launch(Dispatchers.Main) {
                delay(delayMs)
                if (_currentStation.value?.id == currentStation.id) {
                    Logger.d(TAG, "Ejecutando reintento $retryCount para ${currentStation.name}")
                    playStation(currentStation)
                }
            }
        } else {
            Logger.e(TAG, "Se agotaron los reintentos para ${currentStation?.name}")
            retryCount = 0 // Reset para próximas reproducciones
        }
    }
    ...
    fun stopPlayback() {
        shouldResumeOnFocusGain = false
        reconnectJob?.cancel() // Cancelar reintentos
        retryCount = 0
    ...
    fun nextStation() {
        try {
            ...
            Logger.e(TAG, "Error navegando a siguiente estación: ${e.message}")
        }
    }
    ...
    fun previousStation() {
        try {
            ...
            Logger.e(TAG, "Error navegando a estación anterior: ${e.message}")
        }
    }
    ...
    private val playerListener = object : Player.Listener {
        override fun onPlaybackStateChanged(playbackState: Int) {
            when (playbackState) {
                ...
                Player.STATE_BUFFERING -> {
                    ...
                    Logger.d(TAG, "Player buffering")
                }
                Player.STATE_IDLE -> {
                    // Stream not initialized
                    _isBuffering.value = false
                    Logger.d(TAG, "Player idle")
                }
            }
        }
        ...
        override fun onPlayerError(error: androidx.media3.common.PlaybackException) {
            ...
            // Usar el sistema de reintentos mejorado
            handlePlaybackError(errorMsg, _errorType.value!!)
        }
    }
